        ...
        if vehModel ~= nil and vehModel ~= 0 then
            if DoesEntityExist(currVeh) then
                if IsThisModelABoat(vehModel) or IsThisModelAJetski(vehModel) or IsThisModelAnAmphibiousCar(vehModel) or IsThisModelAnAmphibiousQuadbike(vehModel) then
                	local finished = exports["ethical-taskbar"]:taskBar(2000,"Toggling Anchor")
...
AddEventHandler("ethical-jobmanager:playerBecameJob", function(job, name)
...
AddEventHandler("client:bed",function()
  ...
  if DoesEntityExist(objFound) then
    ...
    sleeping = true
    showInteraction = true
	TriggerEvent('ethical-textui:ShowUI', 'show', '[E] Leave Bed')
    ...
    if GetEntityModel(objFound) == -289946279 or GetEntityModel(objFound) == -1519439119 then
      ...
      while sleeping do
        ...
        if not showInteraction then
		 TriggerEvent('ethical-textui:HideUI')
        end
        Citizen.Wait(1000)
    end
...
RegisterNetEvent("ethical-hud:changeRange")
AddEventHandler("ethical-hud:changeRange", function(pRange)
