...
AddEventHandler("houserobberies:item", function(searchedpreviously)
    ...
    if searchedpreviously then
        TriggerEvent('DoLongHudText', 'You already search here!',2)
    else
...
AddEventHandler("breach", function(isFlashbang)
    ...
    if isFlashbang then
        if exports["ethical-inventory"]:hasEnoughOfItem("-73270376",1,false) then
...
AddEventHandler("robbery:lockpickhouse", function(isForced)
   ...
   trailer = false
    ...
    Wait(1500)
    local finished = exports["ethical-lockpicking"]:lockpick(100,5,2,10)
...
function PawnItems(shopnumber)
    ...
    while i < 98 do
        ...
        local itemcount = exports["ethical-inventory"]:getQuantity(pawnTable[i])
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if dstOffice < 3.0 then
            DrawText3Ds( 172.78, -26.89, 68.35, 'ethical-Job Centre' )
...
--  DoScreenFadeOut(1000)
    if isFlashbang and generator then
        DoScreenFadeOut(1)
        local finished = exports["ethical-taskbar"]:taskBar(2000,"Throwing Flash")
        if finished == 100 then
